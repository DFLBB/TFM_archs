...
func (tcc *ThisChainCode) registrarAfijo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	// args[0] --> DatosAfijo (tipoRegistrarAfijo) --- Ejemplo: {"Nombre":"Coramonte","Propietarios":[{"IDPersona":1},{"IDPersona":2},{"IDPersona":3},{"IDPersona":4}]}
	// arg[1] --> IDPersonaEjecuta (int) --- Ejemplo: 123
...
func (tcc *ThisChainCode) registrarCancelacionAfijo(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	type tipoQueryAfijos struct {
		Key    string        `json:"Key"`
		Record cc_cfg.Afijos `json:"Record"`
	}
	...
	var datosAfijo tiporegistrarCancelacionAfijo
	err := json.Unmarshal(DatosAfijoComoJson, &datosAfijo)
	if err != nil {
		fmt.Println(err)
	}
	...
	var datosRegistro []tipoQueryAfijos
