...
func (tcc *ThisChainCode) cargarDatosIniciales_VacunasPerrosProteccion(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for _, nuevoRegistro := range nuevosRegistros {
		...
		err = stub.PutState(cc_cfg.CFG_ObjectType_VacunasProteccion+strconv.Itoa(nuevoRegistro.IDVacunaPerroProteccion), nuevoRegistroAsBytes)
		if err != nil {
			return shim.Error(err.Error())
...
func (tcc *ThisChainCode) registrarVacunaPerro(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	type tipoRegistrarVacunaPerroPropietario struct {
		IDProteccion int    `json:"IDProteccion"`
	...
	queryString = "{\"selector\":{\"docType\":\"" + cc_personas_cfg.CFG_ObjectType + "\",\"IDPersona\":" + strconv.Itoa(IDPersonaVeterinario) + "}}"
	...
	if string(response.Payload) == "[]" {
		return shim.Error("(Args[0]) IDPersonaVeterinario: [ " + strconv.Itoa(IDPersonaVeterinario) + " ] no existe o no es valido")
	}
	...
	for proteccion := range Protecciones {
		...
		IDProteccion := Protecciones[proteccion].IDProteccion

		// ---------------------------------------------------------------------------------------------------

		queryString = "{\"selector\":{\"docType\":\"" + cc_cfg.CFG_ObjectType_VacunasProteccion + "\",\"IDProteccion\":" + strconv.Itoa(IDProteccion) + "}}"
		// fmt.Println(queryString)
		...
		if string(queryResults) != "[]" {
			return shim.Error("(Args[0, " + strconv.Itoa(proteccion) + "]) IDProteccion: [ " + strconv.Itoa(IDProteccion) + " ] no existe o no es valido")
		}
	...
	for proteccion := range Protecciones {
		...
		IDVacunaProteccion := Protecciones[proteccion].IDProteccion
