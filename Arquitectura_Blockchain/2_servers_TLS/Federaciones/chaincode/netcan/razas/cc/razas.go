...
import (
	...
	cc_cfg "github.com/chaincode/netcan/razas/lib/razas_cfg"
...
func (tcc *ThisChainCode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	} else if function == "cargarDatosIniciales_Grupos" {
		return tcc.cargarDatosIniciales_Grupos(stub, args)
...
func (tcc *ThisChainCode) cargarDatosIniciales(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var nuevosRegistros []cc_cfg.Razas
	...
	for _, nuevoRegistro := range nuevosRegistros {
		...
		err = stub.PutState(cc_cfg.CFG_ObjectType+strconv.Itoa(nuevoRegistro.IDRaza), nuevoRegistroAsBytes)
		if err != nil {
			return shim.Error(err.Error())
		}
	}
...
func (tcc *ThisChainCode) cargarDatosIniciales_Grupos(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	var nuevosRegistros []cc_cfg.Grupos
	json.Unmarshal(nuevosRegistrosAsJson, &nuevosRegistros)
	...
	for _, nuevoRegistro := range nuevosRegistros {
		...
		err = stub.PutState(cc_cfg.CFG_ObjectType_Grupos+strconv.Itoa(nuevoRegistro.IDGrupo), nuevoRegistroAsBytes)
		if err != nil {
			return shim.Error(err.Error())
		}
	}
