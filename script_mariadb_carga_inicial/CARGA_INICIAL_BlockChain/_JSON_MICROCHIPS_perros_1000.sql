...
-- Volcando estructura para procedimiento CARGA_INICIAL_BlockChain._JSON_MICROCHIPS_perros_1000
DELIMITER //
CREATE PROCEDURE `_JSON_MICROCHIPS_perros_1000`()
BEGIN
	DECLARE numero_ficheros INT; 
	DECLARE numero_registrosXfichero INT; 
	
	SET numero_registrosXfichero = 1000; 
	SET numero_ficheros = (SELECT CEIL(COUNT(*)/numero_registrosXfichero) FROM MICROCHIPS_perros); 
	
	FOR fichero IN 1..numero_ficheros DO
		SELECT CONCAT(	'['
						, GROUP_CONCAT( '{'  
											, CONCAT( '"', 'ObjectType',				'":', '"', 	'MICROCHIPS_PERROS'		, '"'	, ',')
											, CONCAT( '"', 'IDMicrochipPerro', 		'":',		 	ID									, ',')
						...
						,']') AS JSON_MICROCHIPS_perros
		FROM	MICROCHIPS_perros
