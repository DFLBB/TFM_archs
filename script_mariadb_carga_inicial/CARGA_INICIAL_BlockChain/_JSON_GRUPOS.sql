...
-- Volcando estructura para procedimiento CARGA_INICIAL_BlockChain._JSON_GRUPOS
DELIMITER //
CREATE PROCEDURE `_JSON_GRUPOS`()
SELECT CONCAT(	'['
				, GROUP_CONCAT( '{'  
									, CONCAT( '"', 'docType',		'":', '"', 	'GRUPOS'						, '"'	, ',')
									, CONCAT( '"', 'IFDGrupo', 		'":',		ID_GRUPO						 		, ',')
									, CONCAT( '"', 'Nombre', 		'":', '"', 	NOMBRE_GRUPO				, '"'	, ',')
