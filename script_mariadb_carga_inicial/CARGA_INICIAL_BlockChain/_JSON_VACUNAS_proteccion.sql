...
-- Volcando estructura para procedimiento CARGA_INICIAL_BlockChain._JSON_VACUNAS_proteccion
DELIMITER //
CREATE PROCEDURE `_JSON_VACUNAS_proteccion`()
SELECT CONCAT(	'['
				, GROUP_CONCAT( '{'  
									, CONCAT( '"', 'docType',						'":', '"', 	'AFIJOS'						, '"'	, ',')
									, CONCAT( '"', 'IDVacunaProteccion', 		'":',		 	ID								 		, ',')
									, CONCAT( '"', 'Nombre', 						'":', '"', 	NOMBRE						, '"'	, ',')
									, CONCAT( '"', 'FechaAlta', 					'":', '"', 	IFNULL(FECHA_ALTA, '')	, '"'	, ',')
									, CONCAT( '"', 'FechaBaja', 					'":', '"',	IFNULL(FECHA_BAJA, '')	, '"'	)
									,'}\n'
									)
				,']') AS JSON_VACUNAS_proteccion
