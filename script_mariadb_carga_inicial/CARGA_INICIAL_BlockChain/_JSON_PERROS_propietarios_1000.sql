...
-- Volcando estructura para procedimiento CARGA_INICIAL_BlockChain._JSON_PERROS_propietarios_1000
DELIMITER //
CREATE PROCEDURE `_JSON_PERROS_propietarios_1000`()
BEGIN
	DECLARE numero_ficheros INT; 
	DECLARE numero_registrosXfichero INT; 
	
	SET numero_registrosXfichero = 1000; 
	SET numero_ficheros = (SELECT CEIL(COUNT(*)/numero_registrosXfichero) FROM PERROS_propietarios); 
	
	FOR fichero IN 1..numero_ficheros DO
		SELECT CONCAT(	'['
						, GROUP_CONCAT( '{'  
											, CONCAT( '"', 'docType',				'":', '"', 	'PERROS_PROPIETARIOS'		, '"'	, ',')
											, CONCAT( '"', 'IDPerroPropietario', 	'":',		 	ID									, ',')
											, CONCAT( '"', 'IDPerro', 					'":',		 	ID_PERRO							, ',')
											, CONCAT( '"', 'IDPersona',				'":', 	 	ID_PROPIETARIO					, ',')
											, CONCAT( '"', 'FechaAlta', 				'":', '"', 	IFNULL(FECHA_ALTA, '')		, '"'	, ',')
											, CONCAT( '"', 'FechaBaja', 				'":', '"',	IFNULL(FECHA_BAJA, '')		, '"'	)
											,'}\n'
											)
						,']') AS JSON_PERROS_propietarios
		FROM	PERROS_propietarios
