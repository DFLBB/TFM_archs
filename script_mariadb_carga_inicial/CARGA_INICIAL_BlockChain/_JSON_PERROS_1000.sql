...
-- Volcando estructura para procedimiento CARGA_INICIAL_BlockChain._JSON_PERROS_1000
DELIMITER //
CREATE PROCEDURE `_JSON_PERROS_1000`()
BEGIN
	DECLARE numero_ficheros INT; 
	DECLARE numero_registrosXfichero INT; 
	
	SET numero_registrosXfichero = 1000; 
	SET numero_ficheros = (SELECT CEIL(COUNT(*)/numero_registrosXfichero) FROM PERROS); 
